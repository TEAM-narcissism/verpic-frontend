...
function ReservationCardList(props) {
      ...
      .then((res) => {
        if (res.data) {
          ...
          setFilteredReservList(
            res.data.filter((reserv) => reserv.topic.studyDay === today)
          );
          setIsLoading(false);
        }
      })
  ...
  function setCurrentPageAndDay(day) {
    setToday(day);
    setFilteredReservList(
      reservation.filter((reserv) => reserv.topic.studyDay === day)
    );
    setCurrentPage(1);
  }
      ...
      {isLoading ? (
        ...
        <div>
          <CardListWrapper>
            ...
            <div class="text-gray-600 mb-3 select-none">
              {t("cardlistlongtext")}
            </div>
