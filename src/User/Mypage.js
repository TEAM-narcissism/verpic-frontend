...
function Mypage() {
  ...
  useEffect(async () => {
    ...
    await axios.get("/api/matching/user/" + id).then((res) => {
      if (res.data) {
        ...
        <>
          ...
          <ProfileWrapper>
            ...
            <div class="text-center border rounded-lg mb-10 bg-white">
              ...
              <ProfileText>
                {" "}
                {t("email")} <span className="font-semibold">{user.email}</span>
              </ProfileText>
              <ProfileText>
            ...
            <ReserveListWrapper>
              {reservationList.length === 0 ? (
                <div class="text-center p-2">{t("noreservedstudy")}</div>
              ) : (
                <></>
              )}
                ...
                <div class="text-center mb-5 mt-5" key={reservation.id}>
                  <div class="font-semibold text-xl">
                    {i18next.language === "kr"
                      ...
                      : reservation.topic.engTheme}
                  </div>

                  <div>{reservation.studyDate}</div>
                    ...
                    <>
                      <div>
                        ...
                        <span>
                          <FontAwesomeIcon
                            icon={faCheckCircle}
                            className="text-green-500 w-5 ml-1 mt-1"
                          />
                        </span>
                      </div>

                      <div
                        class="border p-1 rounded-lg w-1/6 my-2 mx-auto cursor-pointer"
